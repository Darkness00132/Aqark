...
export class ImageService {
    ...
    async processAdImages(files) {
        try {
            return await pMap(files, async (file) => this.processAdImage(file.buffer), { concurrency: 5 });
        }
        catch (error) {
    ...
    validateFiles(files, type, maxCount) {
        // Check count
